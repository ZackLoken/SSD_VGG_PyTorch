...
Sample predictions for four images in the test dataset. Predictions have been post-processed using non- 439 maximum suppression and filtered to remove any predictions with confidence scores below 0.5. For each detection, 440 the model predicts the class, location, and confidence score. The model correctly predicted the locations and classes 441 of one American coot and three northern shoveler (top left), 11 mallard (top right), six green-winged teal (bottom left), 442 and 19 gadwall and one ring-necked duck (bottom right).
